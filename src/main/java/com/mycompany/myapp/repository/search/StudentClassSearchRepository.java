...
interface StudentClassSearchRepositoryInternal {
    Stream<StudentClass> search(String query);

    Stream<StudentClass> search(Query query);
...
class StudentClassSearchRepositoryInternalImpl implements StudentClassSearchRepositoryInternal {
    ...
    @Override
    public Stream<StudentClass> search(String query) {
        NativeQuery nativeQuery = new NativeQuery(QueryStringQuery.of(qs -> qs.query(query))._toQuery());
        return search(nativeQuery);
    }

    @Override
    public Stream<StudentClass> search(Query query) {
        return elasticsearchTemplate.search(query, StudentClass.class).map(SearchHit::getContent).stream();
    }
